...
<div class="container mx-auto px-4">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-stone-800">Categorias</h1>
        <a href="{{ route('categorias.create') }}" class="bg-amber-800 hover:bg-amber-900 text-white font-bold py-2 px-4 rounded-lg flex items-center transition-transform duration-300 ease-in-out transform hover:scale-105">
        ...
        <table class="min-w-full leading-normal">
            <thead>
                <tr class="bg-gray-50">
                    ...
                    <th class="px-6 py-4 border-b-2 border-gray-200 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Total de Produtos</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 font-medium">{{ $categoria->nome }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                        <span class="bg-gray-200 text-gray-800 py-1 px-3 rounded-full text-xs font-semibold">{{ $categoria->produtos_count }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <div class="flex items-center space-x-3">
                            <a href="{{ route('categorias.edit', $categoria->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold p-2 rounded-lg inline-flex items-center transition-transform duration-300 ease-in-out transform hover:scale-110">
                            ...
                            <button type="button" class="cursor-pointer bg-red-500 hover:bg-red-600 text-white font-bold p-2 rounded-lg inline-flex items-center delete-btn transition-transform duration-300 ease-in-out transform hover:scale-110" data-url="{{ route('categorias.destroy', $categoria->id) }}">
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-10 text-center text-gray-500">Nenhuma categoria encontrada.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="deleteModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center hidden z-50 transition-opacity duration-300">
    <div class="bg-white p-8 rounded-2xl shadow-2xl w-11/12 md:w-1/3 transform transition-transform duration-300 scale-95">
        ...
        <p class="text-gray-600">Você tem certeza que deseja excluir esta categoria?</p>
