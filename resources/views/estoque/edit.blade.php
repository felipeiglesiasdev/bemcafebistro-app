...
<div class="container mx-auto px-4">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-700">Editar Registro de Estoque</h1>
        ...
        <form action="{{ route('estoque.update', $estoque->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="quantidade" id="quantidade" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('quantidade', $estoque->quantidade) }}">
                </div>

                <div class="mb-4">
                    <label for="preco_compra" class="block text-gray-700 text-sm font-bold mb-2">Preço de Compra:</label>
                    <input type="text" name="preco_compra" id="preco_compra" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('preco_compra', number_format($estoque->preco_compra, 2, ',', '.')) }}">
                </div>

                <div class="mb-4">
                    <label for="preco_venda" class="block text-gray-700 text-sm font-bold mb-2">Preço de Venda:</label>
                    <input type="text" name="preco_venda" id="preco_venda" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('preco_venda', number_format($estoque->preco_venda, 2, ',', '.')) }}">
                </div>

                <div class="mb-4">
                    <label for="data_compra" class="block text-gray-700 text-sm font-bold mb-2">Data da Compra:</label>
                    <input type="date" name="data_compra" id="data_compra" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('data_compra', $estoque->data_compra) }}">
                </div>

                <div class="mb-4">
                    <label for="data_validade" class="block text-gray-700 text-sm font-bold mb-2">Data de Validade (Opcional):</label>
                    <input type="date" name="data_validade" id="data_validade" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('data_validade', $estoque->data_validade) }}">
                </div>
            </div>
